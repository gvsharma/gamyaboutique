...
JAR_KEY="${DEPLOY_OBJECT_KEY:-incoming/gamya-couture.jar}"
SCRIPT_KEY="${DEPLOY_SCRIPT_KEY:-incoming/remote-deploy.sh}"
SYNC_KEY="${DEPLOY_SYNC_SCRIPT_KEY:-incoming/sync-rds-env-from-ssm.sh}"
...
main() {
  mkdir -p "${APP_PATH}/incoming" "${APP_PATH}/scripts" "${APP_PATH}/app" \
  ...
  aws s3 cp "s3://${DEPLOY_BUCKET}/${SYNC_KEY}" "${APP_PATH}/scripts/sync-rds-env-from-ssm.sh"
  aws s3 cp "s3://${DEPLOY_BUCKET}/${SCRIPT_KEY}" "${APP_PATH}/scripts/remote-deploy.sh"
  chmod 755 "${APP_PATH}/scripts/sync-rds-env-from-ssm.sh" "${APP_PATH}/scripts/remote-deploy.sh"
